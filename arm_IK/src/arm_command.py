...
class SteerClaw:
    ...
    def __init__(self, address, dev_name, baud_rate, name, deadzone1 = 2, deadzone2 = 2, sample_time=0.1, last_time=0.00, current_time=0.00):
        ...
        self.targetLength1=0.00
        self.targetLength2=0.00
    ...
    def update_rpm(self):
        ...
        if(target_theta==-1):
            ...
            if(target_L1<25 or target_L2<25):
                target_state=curr_state
                print("Actuator length below 25 indication")
            else:
                target_state=np.array([target_L1,target_L2])
        if(self.deltax!=0 or self.deltay!=0):
            print("To move | deltax="+str(self.deltax)+" deltay="+str(self.deltay))
            # print("To move to | target theta="+str(target_theta)+" target phi="+str(target_phi))
            # print("Current State: | " +str(estimated_theta)+" | "+str(estimated_phi))
            print("Current Legnth: | " +str(estimated_L1)+" | "+str(estimated_L2))
            print("Target Legnth: | " +str(target_L1)+" | "+str(target_L2))
            print("Net Displacement required "+str((target_state-curr_state)/2.54))
        deriv=(target_state-curr_state)*200

        if(self.deltay!=0):
            if(self.deltay>0):
                deriv[0]=deriv[0]*4.35
                deriv[1]=deriv[1]*2.5
            else:
                deriv[0]=deriv[0]*4.35
                deriv[1]=deriv[1]*2.5
        ...
        if(self.deltax!=0):
            if(self.deltax>0):
                deriv[1]=deriv[1]*1.68
                deriv[0]=deriv[0]*1
            else:
                deriv[1]=deriv[1]*1.9
                deriv[0]=deriv[0]*1.2  
        if(self.deltax!=0 or self.deltay!=0):
            print("Vel: | " +str(int(deriv[0]))+" | "+str(int(deriv[1])))
        velM1=int(deriv[0])
...
def steer_callback(inp):
    #minus due to actuator convention
    roboclaw1.deltax=3*inp.data[0]
    roboclaw1.deltay=3*inp.data[1]
